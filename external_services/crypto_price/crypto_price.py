...
from config_data.config import CRYPTO_EXCHANGES, DEFAULT_CURRENCY
...
async def get_crypto_prices(crypto_symbol: str) -> dict:
    ...
    """
    logger.info("You are in def get_crypto_prices!!!")
    ...
    symbol = f"{crypto_symbol}/{DEFAULT_CURRENCY}"
    ...
    for exchange_name in CRYPTO_EXCHANGES:
        ...
        try:
            ...
            # logger.debug(f"Price of {crypto_symbol} on {exchange_name}: {last_price}")
